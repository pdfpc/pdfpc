...
\begin{frame}
  \frametitle{Controlling the timer}
  \begin{itemize}
    \item With \keys{S} you can start the timer at the beginning of the
      presentation
      \singleitem{Note that the timer also starts automatically when moving
        slides}
    \item With \keys{P} you can pause (continue) the timer
      \singleitem{Useful for rehearsal talks}
    \item Press \keys{\shift+T} to cycle between the timer view modes
        (count up/count down/current time)
...
\begin{frame}
  ...
  \begin{itemize}
    \item If you started \pdfpc with the presenter view in the windowed mode,
      it can be toggled to the fullscreen mode (and back) with \keys{W}
    \item You may wish to maximally enlarge the main, current slide
      view sometimes (e.g., to increase accuracy while drawing)
      \singleitem{Press \keys{shift+A} to toggle this feature}
    \item A general customization is enabled by pressing \keys{shift+C}
      \singleitem{In this mode, mouse-draggable handles will appear}
      \singleitem{Exit the customization with \keys{esc}}
  \end{itemize}
\end{frame}
...
\begin{frame}
  ...
  \only<1>{
    \setbeamercolor{block body}{bg=red!40!blue!30}
    \begin{block}{This slide will dissolve}
      \vskip 4cm \hfill
    \end{block}
  }
  \only<2>{
    \setbeamercolor{block body}{bg=blue!40!green!30}
    \begin{block}{This slide will be covered from above}
      \vskip 4cm \hfill
    \end{block}
  }
  \only<3>{
    \setbeamercolor{block body}{bg=green!40!red!30}
    \begin{block}{This slide will be pushed to the left}
      \vskip 4cm \hfill
    \end{block}
  }

  \transdissolve<2>
  \transcover[direction=90]<3>
\end{frame}

\begin{frame}
  \transpush<1>[direction=180]
  ...
  \transduration<2-12>{0.2}
  {\Huge\hfill
  \onslide<2->H\onslide<3->e\onslide<4->l\onslide<5->l\onslide<6->o\onslide<7->,
  \onslide<8->w\onslide<9->o\onslide<10->r\onslide<11->l\onslide<12->d\onslide<13->!
  \hfill}
\end{frame}
