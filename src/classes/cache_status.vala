...
namespace pdfpc {
    ...
    public class CacheStatus
    {
         ...
         */
        public void monitor_renderer(Renderer.Pdf renderer) {
            renderer.prerendering_started.connect((v) => {
                this.max_value += (int) renderer.metadata.get_slide_count();
            });
            renderer.slide_prerendered.connect(() => {
                ++this.current_value;
                this.update();
            });
        }
